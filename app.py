import os
import requests
from flask import Flask, request, jsonify

app = Flask(__name__)

# CONFIGURAÇÕES
DIFY_API_URL = "https://api.dify.ai/v1/chat-messages"
DIFY_API_KEY = "app-Akh9PYeeurIE4J8bcYzqAIcx" # A que você gerou lá

Aqui está o código completo e final para o seu app.py. Ele já inclui a lógica de autenticação segura usando a variável de ambiente do Render e o tratamento correto para o formato que o Vapi envia.

O Código Final (app.py)
Substitua todo o conteúdo do seu arquivo no GitHub por este código:

Python
import os
import requests
from flask import Flask, request, jsonify

app = Flask(__name__)

# Configurações obtidas das variáveis de ambiente do Render
DIFY_API_KEY = os.environ.get("DIFY_API_KEY")
DIFY_API_URL = "https://api.dify.ai/v1/chat-messages"

@app.route('/chat', methods=['POST'])
def chat():
    # Recebe os dados do Vapi
    data = request.json
    
    # Extrai a mensagem enviada pelo usuário via Vapi
    try:
        user_message = data['message']['content']
    except (KeyError, TypeError):
        return jsonify({"message": {"role": "assistant", "content": "Erro: formato de mensagem inválido."}}), 400
    
    # Prepara o payload para o Dify
    payload = {
        "inputs": {},
        "query": user_message,
        "response_mode": "blocking",
        "user": "vapi_kaori_user"
    }
    
    # Cabeçalho com a Chave de API configurada no Render
    headers = {
        "Authorization": f"Bearer {DIFY_API_KEY}", 
        "Content-Type": "application/json"
    }
    
    # Chama o Dify
    try:
        response = requests.post(DIFY_API_URL, headers=headers, json=payload)
        response.raise_for_status() # Lança erro se a resposta não for 200
        dify_data = response.json()
        ai_response = dify_data.get("answer", "...")
    except Exception as e:
        ai_response = "Desculpe, estou com problemas técnicos no momento."
    
    # Retorna o formato que o Vapi espera para falar
    return jsonify({
        "message": {
            "role": "assistant",
            "content": ai_response
        }
    })

if __name__ == '__main__':
    port = int(os.environ.get("PORT", 5000))
    app.run(host='0.0.0.0', port=port)
